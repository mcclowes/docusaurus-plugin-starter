...
import pluginStarter from '../src/index'

const context = { siteDir: '/tmp' } as unknown as LoadContext
...
describe('pluginStarter', () => {
  ...
  it('wires contentLoaded helpers', async () => {
    ...
    const content = await plugin.loadContent!()
    const createData = vi.fn(() => Promise.resolve('/generated/data.json'))
    ...
    expect(createData).toHaveBeenCalledWith('starter-data.json', expect.stringContaining('"Hi"'))
    expect(addRoute).toHaveBeenCalledWith(
      expect.objectContaining({ path: '/starter', exact: true })
    )
